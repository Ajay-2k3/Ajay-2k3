...
My work spans **React / Next.js applications, REST APIs, Node.js services, PostgreSQL, Redis, Supabase, WebSockets, authentication, database optimization, and Python-based AI services**.
...
`React` `TypeScript` `Node.js` `Express` `FastAPI` `Python` `PostgreSQL` `Redis` `Socket.IO`
...
`React Native` `Expo` `Redux Toolkit` `React Query` `Supabase` `PostgreSQL`
...
`Next.js` `React` `TypeScript` `PostgreSQL` `Prisma` `Redis` `Socket.IO`
...
**JPMorgan Chase Software Engineering Virtual Experience / Job Simulation — Forage**
...
<sub>FULL-STACK DEVELOPER · SOFTWARE ENGINEER · CHENNAI</sub>
